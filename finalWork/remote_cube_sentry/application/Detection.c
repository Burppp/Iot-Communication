...
//                    }
//                }
                if(BUZZER_REMIND_ENABLE && detect_list[DETECT_GIMBAL_DM_YAW].status == OFFLINE)
                {
                    if(buzzer_remind_count==5||buzzer_remind_count==10)
                    {
                        buzzer_remind();
...
//    detect_init(DETECT_REFEREE,200,1);
    ...
    for(uint8_t i=DETECT_CHASSIS_6020_RF;i<=DETECT_CHASSIS_6020_LB;i++)
    {
        detect_init(i, 200, 2);
    }

    detect_init(DETECT_GIMBAL_DM_YAW, 200, 4);
    detect_init(DETECT_GYRO_UART, 100, 4);
    detect_init(DETECT_CAP, 200, 3);
